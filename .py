...
def gen(delay, audio, video, timestamp):
    ...
    s_mX0 = [(v + u // 600 + 1) % 6 for v in range(6)]
    g_mX = ";".join(
        f"amovie=cycle_s60_{i:02d}.wav,asplit={len(v)}{''.join(f'[m{n:02d}]' for n in v)}"
        for i, v in enumerate(c)
    )
    g_mmX = ";".join(f"amovie=minute_{v}0.wav,aloop=9:44100*60[m{v}X]" for v in s_mX0)
    g_hX = ";".join(f"amovie=hour_{v:02d}.wav,aloop=59:44100*60[h{v:02d}]" for v in s_h)
    ...
    g_m = f"{''.join(f'[m{v}X]'for v in s_mX0)}concat=v=0:a=1:n=6,aloop=-1:44100*60*60[mX0]"
    g_mb = f"amovie=minute_{u//600%6}0.wav,aloop={9-u//60%10}:44100*60,atrim={u%60}[mb]"
    g_mm = f"[mb][mX0]concat=v=0:a=1:n=2[m]"
    g_hh = f"{''.join(f'[m{v:02d}]'for v in s_m0X)}concat=v=0:a=1:n=60,aloop=-1:44100*60*60,atrim={u%60}[m0X]"
    g_hb = f"amovie=hour_{u//3600%24:02d}.wav,aloop={59-u//60%60}:44100*60,atrim={u%60}[hb]"
    ...
    g_a = f"[m0X][s600][s60][m][h]amix=inputs=5,volume=5,asetpts=PTS-STARTPTS"
    ...
    segs = filter(
        None,
        [
            f"{g_mX};{g_mmX};{g_hX};{g_s600};{g_s60};{g_m};{g_mb};{g_mm};{g_hh};{g_hb};{g_dX};{g_h};{g_a}"
            if audio
            else None,
